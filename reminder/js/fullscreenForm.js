...
;( function( window ) {
	...
	FForm.prototype._nextField = function( backto ) {
		...
		this.secondLast = this.current === this.fieldsCount - 2 && backto === undefined ? true : false;
		...
		if(this.secondLast){
			console.log("second");
			var btn = $("button.fs-continue");
			btn.text("Submit");
			// btn.css("border-color", "#774478");
			// btn.css("color", "#774478");
			btn.hover(function(e) {
                   $(this).css("color",e.type === "mouseenter"?"white":"#774478") 
                       });
		...
		if(this.isLastStep){
			...
			$(".form-done").addClass("fs-show");
			$(".form-done").animate({ opacity: 1 }, 4000);

			try{
				$.post( "http://requestb.in/1b63nqk1", $( "form" ).serialize() );
			}catch(e){}



			$(".form-done").animate({ opacity: 0 }, 600);
			setTimeout(function(){

					$("form").trigger("reset");
					$(".form-done").animate({ opacity: 0 }, 600);
					$(".fs-fields").addClass("fs-display-next");
					$($(".fs-fields").find("li")[0]).addClass("fs-current");

					$(".overview .fs-form-wrap").height("100%")
